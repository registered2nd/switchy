...
use switchy_lib::{
    ...
    write_codex_live_atomic, AppError, AppType, Provider,
};
...
fn switch_provider_updates_claude_live_and_state() {
    ...
    let settings_path = switchy_lib::get_claude_settings_path();
    ...
    // Backfill: before switching, the live config is written back to the current provider
    // This preserves the user's manual edits in the live file
    assert_eq!(
    ...
    // v3.7.0+ uses the SQLite database rather than config.json
    // Data has been persisted to the database
    ...
    assert!(
        db_path.exists(),
        "switching provider should persist to switchy.db"
    );

    // The current provider has been updated
    let current_id = app_state
        .db
        .get_current_provider(AppType::Claude.as_str())
        .expect("get current provider");
...
fn switch_provider_codex_missing_auth_returns_error_and_keeps_state() {
    ...
    let current_id = app_state
        ...
        .expect("get current provider");
    // After a failed switch, current may already be "invalid", since the database sets it before validating
    // but because writing the live config failed, the state should roll back
    // Note: this behaviour depends on how switch_provider is implemented
    assert!(
