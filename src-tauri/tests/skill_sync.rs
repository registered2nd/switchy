...
fn sync_to_app_removes_disabled_and_orphaned_ssot_symlinks() {
    ...
    let ssot_dir = home.join(".switchy").join("skills");
...
fn uninstall_skill_creates_backup_before_removing_ssot() {
    ...
    let ssot_skill_dir = home.join(".switchy").join("skills").join("backup-skill");
...
fn restore_skill_backup_restores_files_to_ssot_and_current_app() {
    ...
    let ssot_skill_dir = home.join(".switchy").join("skills").join("restore-skill");
