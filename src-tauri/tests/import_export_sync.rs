...
fn sync_enabled_to_codex_writes_enabled_servers() {
    ...
    let path = switchy_lib::get_codex_config_path();
    ...
    switchy_lib::sync_enabled_to_codex(&config).expect("sync codex");
...
fn sync_enabled_to_codex_returns_error_on_invalid_toml() {
    ...
    let err = switchy_lib::sync_enabled_to_codex(&config).expect_err("sync should fail");
    match err {
        switchy_lib::AppError::Toml { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "path should reference config.toml"
            );
        }
        switchy_lib::AppError::McpValidation(msg) => {
            assert!(
...
fn sync_codex_provider_missing_auth_returns_error() {
    ...
    match err {
        switchy_lib::AppError::Config(msg) => {
    ...
    // 确认未产生任何 live 配置文件
    assert!(
        !switchy_lib::get_codex_auth_path().exists(),
        "auth.json should not be created on failure"
    );
    assert!(
        !switchy_lib::get_codex_config_path().exists(),
...
"#,
    ...
    let mut config = MultiAppConfig::default();
    let changed = switchy_lib::import_from_codex(&mut config).expect("import codex");
...
fn sync_claude_enabled_mcp_projects_to_user_config() {
    ...
    switchy_lib::sync_enabled_to_claude(&config).expect("sync Claude MCP");

    let claude_path = switchy_lib::get_claude_mcp_path();
...
fn import_from_claude_merges_into_config() {
    ...
    let changed = switchy_lib::import_from_claude(&mut config).expect("import from claude");
...
fn create_backup_skips_missing_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_path = home.join(".switchy").join("config.json");
...
fn create_backup_generates_snapshot_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_dir = home.join(".switchy");
...
fn sync_gemini_packycode_sets_security_selected_type() {
    ...
    // security field is written to ~/.gemini/settings.json, not ~/.switchy/settings.json
