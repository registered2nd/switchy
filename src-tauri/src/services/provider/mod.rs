...
mod tests {
    ...
    fn with_test_home<T>(test: impl FnOnce(&AppState, &Path) -> T) -> T {
        ...
        let old_test_home = std::env::var_os("SWITCHY_TEST_HOME");
        let old_home = std::env::var_os("HOME");
        std::env::set_var("SWITCHY_TEST_HOME", temp.path());
        ...
        match old_test_home {
            Some(value) => std::env::set_var("SWITCHY_TEST_HOME", value),
            None => std::env::remove_var("SWITCHY_TEST_HOME"),
        }
        match old_home {
...
impl ProviderService {
    ...
    fn switch_inner(
        ...
        Self::add_to_switching_order(state, &app_type, id)?;
    ...
    /// which puts the current provider first; a provider enabled by hand joins
    /// it if it is not already there. Switching automatically still moves off
    /// it when a request fails or its account is spent.
    fn add_to_switching_order(
        state: &AppState,
        app_type: &AppType,
        id: &str,
    ) -> Result<(), AppError> {
