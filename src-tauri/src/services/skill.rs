...
impl SkillService {
    ...
    /// 获取 Skill 卸载备份目录（~/.switchy/skill-backups/）
    ...
    /// 扫描未管理的 Skills
    ///
    /// 扫描各应用目录，找出未被 Switchy 管理的 Skills
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "switchy".to_string()));
        }
    ...
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "switchy".to_string()));
