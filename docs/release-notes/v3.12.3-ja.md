...
- バックアップは `~/.switchy/skill-backups/` に保存され、すべての skill ファイルと元のメタデータを含む `meta.json` が含まれます
...
この機能を有効にすることで、ユーザーは**すべてのリスクを自己責任で負う**ものとします。Switchy は、この機能の使用に起因するアカウント制限、警告、またはサービス停止について一切の責任を負いません。

---

## ダウンロードとインストール

[Releases](https://github.com/registered2nd/switchy/releases/latest) から適切なバージョンをダウンロードしてください。
...
| ファイル                                   | 説明                                                 |
| ------------------------------------------ | ---------------------------------------------------- |
| `Switchy-v3.12.3-Windows.msi`            | **推奨** - MSI インストーラー、自動更新対応          |
| `Switchy-v3.12.3-Windows-Portable.zip`   | ポータブル版、解凍して実行、レジストリ書き込みなし   |

### macOS

| ファイル                           | 説明                                                              |
| ---------------------------------- | ----------------------------------------------------------------- |
| `Switchy-v3.12.3-macOS.dmg`     | **推奨** - DMG インストーラー、ドラッグ＆ドロップでインストール   |
| `Switchy-v3.12.3-macOS.zip`     | 解凍して Applications にドラッグ、Universal Binary                |
| `Switchy-v3.12.3-macOS.tar.gz`  | Homebrew インストールと自動更新用                                 |
...
| --------------------------------------- | ----------- | ---------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`      | `sudo dpkg -i Switchy-*.deb` または `sudo apt install ./Switchy-*.deb` |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`      | `sudo rpm -i Switchy-*.rpm` または `sudo dnf install ./Switchy-*.rpm`  |
| openSUSE                                | `.rpm`      | `sudo zypper install ./Switchy-*.rpm`                                |
| Arch Linux / Manjaro                    | `.AppImage` | 実行権限を追加して直接実行、または AUR を使用                          |
| その他のディストリビューション / 不明   | `.AppImage` | `chmod +x Switchy-*.AppImage && ./Switchy-*.AppImage`              |
