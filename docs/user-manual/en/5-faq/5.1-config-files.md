...
## Switchy Data Storage

### Storage Directory

Default location: `~/.switchy/`
...
```
~/.switchy/
├── switchy.db      # SQLite database (SSOT)
...
`switchy.db` is a SQLite database that stores:
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to Switchy's "providers") |
...
Switchy's priority when modifying configurations:

1. **Switchy Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by Switchy)
- Switchy's `settings.json`

### Not Recommended to Edit Manually

- `switchy.db` database file
- Backup files
...
Switchy v3.7.0 migrated from JSON files to SQLite:
