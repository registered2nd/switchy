...
Switchy for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/registered2nd/switchy/releases).
...
2. Add Switchy to your antivirus software's whitelist
...
chmod +x Switchy-*.AppImage

# If it still fails, try
./Switchy-*.AppImage --no-sandbox
```
...
**Solutions**:
1. Check if the `~/.switchy/` directory exists
2. Restore from backup: `~/.switchy/backups/`
...
**Solutions**:
1. Confirm the file was exported by Switchy
...
**Windows**:
- Check taskbar settings to ensure the Switchy icon is not hidden
...
2. Restart the app
3. Delete `~/.switchy/settings.json` to reset settings
...
3. If using Homebrew: `brew upgrade --cask switchy`

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and Switchy runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
1. Visit [GitHub Issues](https://github.com/registered2nd/switchy/issues)
...
- macOS/Linux: `~/.switchy/logs/`
- Windows: `%APPDATA%\switchy\logs\`
