...
3. Switchy calls the provider's `/v1/models` endpoint to retrieve the model list
...
> Authentication type is automatically detected by Switchy (PackyCode API proxy / Google OAuth / generic API Key), no manual configuration needed.
...
Switchy supports two ways to import provider configurations:

### Option 1: Deep Link Import

One-click import via `switchy://` protocol links:

1. Click or visit the deep link
2. Switchy opens automatically and shows the import confirmation
3. Preview the configuration information
4. Click "Confirm Import"

**Getting deep links**:
- Obtain from shared links by others
- Create using the [online generator tool](https://registered2nd.github.io/switchy/deplink.html)
...
> **Note**: Importing will overwrite the existing database. It is recommended to export your current configuration as a backup first. The exported file name format is `switchy-export-{timestamp}.sql`.
