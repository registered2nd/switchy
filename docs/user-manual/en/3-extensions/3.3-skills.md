...
Switchy comes pre-configured with the following GitHub repositories:
...
Switchy provides powerful search and filter features:
...
- **Automatic backup**: Before deletion, the skill is backed up to `~/.switchy/skill-backups/`
- Removes the skill from all app directories (Claude, Codex, Gemini, OpenCode)
- Removes the skill from the SSOT directory (`~/.switchy/skills/`)
