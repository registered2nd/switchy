...
This section describes how to configure Switchy according to your preferences.
...
Switchy supports three languages:
...
When enabled, Switchy automatically runs when the system starts.
...
When enabled, Switchy automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that Switchy uses when opening a terminal.
...
The storage location for Switchy's own data, defaulting to `~/.switchy/`.
...
Configure Switchy's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, Switchy automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `switchy-sync`) |
...
- Switchy automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, Switchy displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current Switchy version number, with support for:
