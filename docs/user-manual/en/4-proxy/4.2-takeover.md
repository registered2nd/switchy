...
App takeover means letting Switchy's proxy intercept and forward a specific application's API requests.
...
When takeover is enabled, Switchy modifies the app's configuration file to point the API endpoint to the local proxy.
...
When disabling takeover, Switchy will:
...
Before enabling takeover, Switchy backs up the original configuration and restores it when disabled.
