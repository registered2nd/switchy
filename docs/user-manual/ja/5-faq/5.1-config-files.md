...
`switchy.db` は SQLite データベースで、以下を保存しています：
...
| フィールド | 説明 |
|------|------|
| `models.providers` | プロバイダー設定（Switchy の「プロバイダー」にマッピング） |
| `env` | 環境変数設定 |
...
Switchy が設定を変更する際の優先順位：

1. **Switchy データベース** - 単一事実源 (SSOT)
...
- CLI ツールの設定ファイル（Switchy がバックフィルする）
- Switchy の `settings.json`

### 手動編集を推奨しないもの

- `switchy.db` データベースファイル
- バックアップファイル
...
Switchy v3.7.0 で JSON ファイルから SQLite に移行しました：
