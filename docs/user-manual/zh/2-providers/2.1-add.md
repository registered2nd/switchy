...
3. Switchy 调用供应商的 `/v1/models` 端点获取模型列表
4. 从按供应商分组的下拉菜单中选择模型
...
> 💡 认证类型由 Switchy 自动检测（PackyCode API 代理 / Google OAuth / 通用 API Key），无需手动配置。
...
1. 点击或访问深度链接
2. Switchy 自动打开并显示导入确认
3. 预览配置信息
4. 点击「确认导入」

**获取深度链接**：
- 从他人分享获取
- 使用 [在线生成工具](https://registered2nd.github.io/switchy/deplink.html) 创建
...
> ⚠️ **注意**：导入会覆盖现有数据库，建议先导出当前配置作为备份。导出的文件名格式为 `switchy-export-{时间戳}.sql`。
