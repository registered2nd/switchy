...
A switch changes only what the provider owns — its endpoint, credentials and model — and leaves the rest of the tool's settings file as it is: hooks, plugins, MCP servers, permissions and anything another tool writes stay. The common config snippet (*Edit common config* in the provider form) is for settings you want Switchy to put in place: saving it applies the change to the tool's settings file.
...
- **Logins stay usable.** The proxy renews a stored login when its access token runs out and hands the renewed login to Codex's own `auth.json` and to the copy restored when the proxy is turned off, under the same newest-valid-login-wins rules as [One login, two installs](#one-login-two-installs). A login OpenAI has rejected is not retried. Enabling an account whose login was rejected signs Codex out the way `codex logout` does, so the next Codex started opens on its sign-in screen; sign in there, or with `codex login`, while that provider is current. Codex has no sign-in inside a running session. A new sign-in, on Windows or in WSL, is stored with the enabled card as soon as it appears, with the proxy on or off; the card then shows the account signed in and its failure count and circuit breaker are cleared.
...
- **`/status` follows the switch (experimental, on by default).** A Codex window keeps the account it started with, so `/status` in an open window would go on showing that account. With Settings → Pool → *Switch Codex's signed-in account* on and Codex routed through the proxy, each Codex window started in WSL runs on its own Codex app-server, started from its terminal by a `codex` shell function Switchy writes to `~/.codex/switchy/codex.sh` and sources from `~/.bashrc`. Switchy connects to each window's server and signs it in to the account in use on every switch, whether picked by hand or by rotation, so `/status` there shows the new account's email and limits right away. Because the server starts in the window's own terminal, the hooks Codex runs keep that terminal's environment, so Orca still sees the window. The server keeps the handed-in login in memory and asks Switchy for a new one when OpenAI refuses it, so Switchy is the only holder renewing it; `auth.json` is left alone. It uses Codex's `chatgptAuthTokens` sign-in, which OpenAI marks unstable, so a Codex update can stop it working; the proxy's switching does not depend on it. It applies to windows started from a terminal opened after it was installed; `codex exec`, `codex login` and the other non-window commands run plain Codex, as does everything while the setting is off. Codex on native Windows is not covered.
