...
- **`/status` follows the switch (experimental, on by default).** A Codex window keeps the account it started with, so `/status` in an open window would go on showing that account. Settings → Pool → *Switch Codex's signed-in account* keeps Codex's shared background server (`codex app-server daemon`) running, one in Windows and one in WSL, and signs it in to the account in use on every switch, whether picked by hand or by rotation. Every `codex` window started while it runs attaches to it on its own, so `/status` there shows the enabled account's email and limits right after a switch, and a new sign-in is picked up as soon as it is filed with its card. It uses Codex's `chatgptAuthTokens` sign-in, which OpenAI marks unstable, so a Codex update can stop it working; the proxy's switching does not depend on it. The server keeps the handed-in login in memory and asks Switchy for a new one when OpenAI refuses it, so Switchy is the only holder renewing it; `auth.json` is left alone. Windows open before it was turned on need one restart. Turning it off, or turning off Codex's routing, stops the server, which closes the windows attached to it; `codex resume` reopens them. Codex's server does not run elevated on Windows.
