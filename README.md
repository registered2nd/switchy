...
- **Rotation.** With *Switch automatically* on (Settings → Pool), the app's switching order is the pool of accounts. *Rotate accounts before the limit* moves an account whose usage has reached the threshold to the back of the order until its limit resets, and an account OpenAI refuses for usage is passed over until the reset it names. When every account is spent the request still goes out, so you see OpenAI's own message.
