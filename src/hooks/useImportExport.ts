...
export function useImportExport(
  ...
  const importConfig = useCallback(async () => {
    ...
    try {
      ...
      // Trigger the external refresh right after import (independent of the live sync result)
      // - so the UI still refreshes when the sync fails
      // - so it does not rely on setTimeout (unmount would cancel it)
