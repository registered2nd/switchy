...
- Files: `src-tauri/src/proxy/manual_hold.rs`; `select_providers` in `proxy/provider_router.rs`; `do_switch` in `proxy/failover_switch.rs`; `save_login_of_serving_account` in `proxy/codex_pool.rs`, called from `forward` in `proxy/forwarder.rs`.
