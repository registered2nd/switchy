...
- Decision:
  ...
  4. **A refused account picked by hand signs Codex out.** When the held account's renewal is refused, its login is removed from Codex's `auth.json` (Windows and the WSL mirror), so Codex opens on its own sign-in screen instead of running on another account's login while every request goes out as the refused one. Stored logins stay with their providers, and a working account's login returns after its first answer. The login is not checked when it is picked: renewing it behind a Codex that holds the same account's login can get the whole login revoked, so the proxy renews only after a 401.
- Why: the user — a manual pick is an override the pool must respect, and `/status` must show it; automatic choice is for when no one has picked. A signed-out account must look to Codex like a signed-out Codex.
- Files: `src-tauri/src/proxy/manual_hold.rs`; `select_providers` in `proxy/provider_router.rs`; `do_switch` in `proxy/failover_switch.rs`; `save_login_of_serving_account`, `keep_live_login` and `sign_codex_out_for` in `proxy/codex_pool.rs`; `forward` in `proxy/forwarder.rs`.
