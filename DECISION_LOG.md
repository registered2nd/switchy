...
- Decision: the Codex takeover also sets `chatgpt_base_url` to the proxy's `/backend-api` (Windows and the WSL mirror; removed with `openai_base_url`). Codex accepts only an HTTPS `chatgpt_base_url` (an `http://` one fails `account/read` at startup and Codex exits), so the proxy also listens on HTTPS one port above its own, with a certificate for 127.0.0.1 and localhost signed by an authority Switchy makes once and whose private key it never stores; the authority is added to the Windows machine Root store (`certutil`) and to WSL's (`update-ca-certificates`, as root). `chatgpt_base_url` is written only for an install that trusts it. Consequence: Codex's built-in apps connector (`codex_apps`, `{chatgpt_base_url}/ps/mcp`) fails with 451 `no_biscuit_no_service`, because Codex fetches its agent-identity token only when `chatgpt_base_url` is one of chatgpt.com's own addresses (`ChatGptEnvironment::from_chatgpt_base_url`). The proxy answers `/backend-api/wham/usage*` with the login of the account it serves; every other `/backend-api/*` call is forwarded to `https://chatgpt.com` unchanged, with the login Codex sent. The account line is left as Codex shows it.
...
- Files: `src-tauri/src/proxy/local_tls.rs`; `start_local_tls` in `proxy/server.rs`; `handle_codex_chatgpt_backend` in `proxy/handlers.rs`; `set_codex_chatgpt_base_url`, `set_codex_openai_base_url` and `codex_takeover_base` in `services/proxy.rs`; `CHATGPT_BACKEND_PATH_PREFIX` and `CHATGPT_ORIGIN` in `proxy/codex_pool.rs`.
