...
## 2026-09-23 — Codex's shared app-server is signed in to the account in use, so an open window's `/status` follows a switch (experimental, on by default)

- Context: proxy mode exists so an open Codex window switches accounts, and the user checks a switch with `/status`. A Codex window keeps its signed-in account for its life: a changed `auth.json` of another account is refused (`reload_if_account_id_matches` in `codex-rs/login/src/auth/manager.rs`), so the proxy moved the requests while `/status` showed the old account. Pointing `chatgpt_base_url` at the proxy changed only the limits `/status` fetched, needed HTTPS, and made Codex skip its agent-identity token (it fetches one only for chatgpt.com's own addresses), which broke its built-in apps connector; it was removed.
- Decision: with Settings → Pool → *Switch Codex's signed-in account* on (default) and Codex routed through the proxy, Switchy starts Codex's shared app-server (`codex app-server daemon start`) in each install, Windows and the WSL mirror's distro, connects to it through `codex app-server proxy` (a WebSocket over stdio), and on every change of the current Codex provider or its access token sends `account/login/start` with `type: chatgptAuthTokens`. Every `codex` window started while the server runs attaches to it without a wrapper, and all of them follow the sign-in. The server keeps the tokens in memory (no `auth.json` write) and sends `account/chatgptAuthTokens/refresh` to this connection on a 401; Switchy answers with a forced renewal of the current provider's login, so it is the only holder renewing it. Turning the setting or Codex's routing off stops the server Switchy started, closing attached windows (`codex resume` reopens them).
- Verified live on codex-cli 0.156.1 in WSL: an open window's `/status` changed from registered2nd (Pro, 100% left) to user-b (Pro Lite, 50% left) on a sign-in, matching `/wham/usage` for each account.
- Why: the user — `/status` must behave as normal Codex, with the account Codex is signed in to actually switched. `chatgptAuthTokens` is marked `[UNSTABLE] FOR OPENAI INTERNAL USE ONLY` in Codex's protocol schema, so it is an experimental setting and the proxy's own switching does not depend on it.
